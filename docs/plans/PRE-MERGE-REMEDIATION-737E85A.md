...
- **PMR-07 — Cockpit recovery re-verification (13/09):** Trên `main` sau PR #262, n8n `2.38.1` local chạy workflow M07 đã import qua Manual Trigger ở execution `131`. Cockpit `gpt-5.6-luna` trả `HUMAN_REVIEW` với hai scalar claim/evidence exact; tất cả node từ tool adapter/context/Agent đến grounding, proposal persistence và report đều `success` trong 13.448 giây. Proposal immutable `sha256:755166a6edaed5622c6c5787201763cd0b2b38d93d569d75858ba140f83bd5a8` được ghi cho canonical record `watch-507d757feb65b05f9d67dd7929fcde805aeb5a89d6ad96b76d29786be0ed292f`; output giữ `A2-RO`, `write_permission=false` và `execution_permitted=false`. Sau restart đúng n8n + adapter loopback, context trả `VALID`, proposal inventory còn đúng một file và history replay `MATCH`. Đây là local synthetic/read-only evidence; không phải selected-source, provider-diversity, business-outcome, deployment hay quyền thực thi.
