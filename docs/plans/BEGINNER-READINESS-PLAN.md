...
| BR-13 | D | Watcher M06 normalize và lưu history thật | P1 / L | BR-09, BR-12 | PARTIAL | M06 fixture builder/resolver chung cho CLI/HTTP, key-order/field ID/DRIFT có regression; chỉ synthetic profile, operated source evidence vẫn mở |
| BR-14 | D | n8n M06 import/smoke/static-data đúng | P1 / M | BR-13 | PARTIAL | Blueprint chỉ gửi fixture sang adapter, ACK sau append+resolve; n8n engine CI kiểm node-chain và Schedule Trigger synthetic/read-only. Chưa có selected deployment/source evidence |
| BR-15 | D | M07 grounding và tool boundary thực thi được | P1 / L | BR-11, BR-14 | PARTIAL | Core/learner adapter kiểm claim/value/ID, adapter-owned tool trace và proposal persistence; CI n8n dùng model stub và local Cockpit synthetic/read-only đã chạy success/restart. Provider diversity và selected source còn mở |
| BR-16 | E | Kiểm thử xuyên hệ thống và pilot người mới | P1 / L | BR-05…BR-15 | PARTIAL | Shared M00–M11 fixture workspace/restart đã có; cần pilot máy sạch, selected source và business outcome trước khi coi người mới tự làm trọn lộ trình |
