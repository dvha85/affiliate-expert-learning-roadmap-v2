...
Runner fixture BR-10 offline IN_REVIEW: Codex thực hiện/review, nhánh `codex/br-11-br10-fixture-runner`; `advisor fixture-run` tạo bundle riêng qua store history/action/outcome → context → mock boundary → advisor.json có output/provenance. Không đọc key/call API, không dùng paid campaign hoặc thay fixture digest v1. Bundle đọc lại được sau lệnh kết thúc; chưa live proof. Hướng dẫn và residual gates trong BR-11B2-DEEPSEEK.md.
