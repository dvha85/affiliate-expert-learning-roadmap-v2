...
| Nơi chạy 24/7 và giới hạn tài nguyên/chi phí | BR-18 | Chủ repo/người vận hành | Kiểm chứng hiện tại trên macOS native; về sau có thể chạy Windows + Ubuntu 24.04 LTS (WSL2 ưu tiên, Hyper-V fallback); chưa mặc định mua VPS và chưa có operated evidence |
...
| BR-10 | C | Tích hợp action/outcome và nhập báo cáo M03 | P1 / M | BR-02, BR-06, BR-09 | IN_PROGRESS | Codex; lab/schema đã nghiệm thu #55 `6d311a8`, [audit](../architecture/BR-10C-ACCEPTANCE.md); BR-10d ACCESSTRADE CSV fixture/importer, hash receipt và backup/restore graph có test; [capability note](../architecture/BR-10D-ACCESSTRADE-CAPABILITY-NOTE.md); live proof còn mở cùng BR-06b |
...
| BR-13 | D | Watcher M06 normalize và lưu history thật | P1 / L | BR-09, BR-12 | PARTIAL | M06 fixture builder/resolver chung cho CLI/HTTP, key-order/field ID/DRIFT có regression. Selected source đầu tiên là ACCESSTRADE Shopee Smartlink, chỉ nhận metadata sanitized/read-only; không fetch, credential hay fact business. Operated source evidence vẫn mở |
| BR-14 | D | n8n M06 import/smoke/static-data đúng | P1 / M | BR-13 | PARTIAL | Blueprint fixture và selected-source inactive/manual chỉ gửi capture sanitized sang adapter, ACK sau append+resolve; n8n engine CI kiểm node-chain và Schedule Trigger synthetic/read-only. Chưa có selected deployment/source evidence |
...
- ACCESSTRADE account và dashboard Publisher đã được kiểm read-only 08/09/2026. Adapter CSV fixture nhận bốn cột dashboard, mapping tường minh order→action, trạng thái hẹp và tiền VND; không dùng UTM thay attribution. Xem [capability note](../architecture/BR-10D-ACCESSTRADE-CAPABILITY-NOTE.md) để phân biệt quan sát dashboard với proof vận hành.
...
- Có fixture export khử dữ liệu riêng tư, tests mapping, receipt hash-only, replay sau backup/restore và thực hành nhập báo cáo; tách synthetic smoke khỏi live proof.
...
BR-11 canary CLI: DONE phạm vi triển khai/kiểm thử offline sau [#67](https://github.com/dvha85/affiliate-expert-learning-roadmap-v2/pull/67) merged `4068979`, head `0871d93`, baseline `da24ce1`. Codex thực hiện/review theo quyền chủ repo; learner tests/race/vet, smoke BR-11a, 8 validators và CI 4/4 PASS. `advisor campaign-canary` chỉ một lượt đầu tiên ở fixed campaign path, key từ môi trường, fixture BR-10 duy nhất, khóa toàn lượt và reserve trước request; lượt sau yêu cầu review kể cả lỗi. Có walkthrough đọc report/đối soát phí và phục hồi không reset. Kiểm hiện diện key ngày 07/09/2026: DEEPSEEK_API_KEY chưa có trong môi trường agent, không đọc/tìm key cá nhân. Chưa khởi tạo campaign thật, gọi API hoặc có proof live. Bước tiếp theo cần key được cấp an toàn trong môi trường chạy (hoặc chủ repo chạy walkthrough rồi cung cấp report đã kiểm loại bí mật) và đối soát phí; BR-11 giữ IN_PROGRESS, không mở BR-12 trước nghiệm thu dependency.

BR-11 BR10 → ledger versioned: DONE phạm vi offline sau [#66](https://github.com/dvha85/affiliate-expert-learning-roadmap-v2/pull/66) merged `f514986`, head `ddd2448`, baseline `611d669`. Codex thực hiện/review theo quyền triển khai/merge của chủ repo. Đã sửa lỗi thứ tự JSON khi đọc lại; learner tests/race/vet, smoke BR-11a, 8 validators và CI 4/4 PASS. Runner nội bộ tạo fixture qua store BR-10, reserve trước provider, lưu result/v2 với context synthetic đóng băng và accepted output; result/v1 giữ nguyên bytes/digest. Regression bảo vệ mixed-version restart, context tampering/rehashed, accepted/rejected output, DeepSeek loopback usage và no-refund. Không CLI live, không reset ledger, không API thật. Bước tiếp theo: CLI một lượt fixture ở fixed campaign path, preflight key và walkthrough, kiểm offline trước canary thật. BR-11 tổng thể giữ IN_PROGRESS.

Runner fixture BR-10 offline DONE sau [#65](https://github.com/dvha85/affiliate-expert-learning-roadmap-v2/pull/65) merged `a26dc34`, head sửa `9693d6e`: Codex thực hiện/review, CI 4/4, learner tests/race/vet và 8 validators PASS. `advisor fixture-run` tạo bundle riêng qua store history/action/outcome → context → mock boundary → advisor.json có output/provenance. Không đọc key/call API, không dùng paid campaign hoặc thay fixture digest v1. Bundle đọc lại được sau lệnh kết thúc; chưa live proof. Bước tiếp theo: nối context BR-10 với campaign có version và accepted-output persistence; giữ ledger cũ không reset. Hướng dẫn và residual gates trong BR-11B2-DEEPSEEK.md. BR-11 giữ IN_PROGRESS.

Campaign init DONE phạm vi offline sau [#64](https://github.com/dvha85/affiliate-expert-learning-roadmap-v2/pull/64) merged `0f5745d`, head `3a96b11`: Codex thực hiện/review; CI 4/4, learner tests/race/vet và 8 validators PASS. CLI `bot advisor campaign-init` dùng user config root cố định, không path argument, không key/network. Kiểm ancestor/owner/quyền trước khi tạo thư mục ứng dụng private và manifest durable; campaign hiện hữu hoặc partial đều từ chối, không reset/chmod/repair. Tests chỉ tạo thư mục tạm. Chưa chạy init thực hoặc live; bước kế tiếp vẫn là runner fixture BR-10 và accepted-output proof. BR-11 giữ IN_PROGRESS.
...
Hiện hành: **BR-12 DONE trong phạm vi lab**, bàn giao/nghiệm thu qua [PR #73](https://github.com/dvha85/affiliate-expert-learning-roadmap-v2/pull/73) (trạng thái merge/CI tại PR). [Walkthrough và ma trận](../architecture/BR-12D-ACCEPTANCE.md), baseline `9dd9070`, implementation `7cf10b3`. Codex thực hiện/review theo quyền chủ repo; local smoke, learner tests/race/vet và 8 validators PASS. Smoke đi M00→M05 qua binary/process mới, giữ INCONCLUSIVE và không execution. Bài sửa nhãn diagnostic có regression FAIL trước/PASS sau, rollback file khớp hash baseline trong bản sao tạm, restore PASS. ReviewRecord của smoke là synthetic REQUEST_CHANGES, không giả human approval hoặc business proof. Trạng thái DONE áp dụng bản kế hoạch được merge qua PR #73; các đoạn IN_PROGRESS bên dưới là lịch sử từng lát cắt. BR-13 là mục kế tiếp trong lab, nguồn thật vẫn cần quyền riêng.
...
BR-14a có [hợp đồng tĩnh M06 n8n](../architecture/BR-14A-N8N-STATIC-CONTRACT.md): validator kiểm tra node/GET-only/change detection/cache boundary/history handoff và chạy trong CI. Disposable n8n `2.38.1` đã chạy fixture synthetic/read-only, nhưng contract/static evidence không thay release admission; `tested_n8n_version` vẫn `UNVERIFIED` cho tới khi có ma trận smoke trên topology/deployment được chọn. BR-14 cha PARTIAL.

BR-14b có [runbook smoke M06](../architecture/BR-14B-N8N-SMOKE-RUNBOOK.md) và case contract offline. CI chạy node chain cùng Schedule Trigger thật trong runtime disposable; runbook vẫn yêu cầu engine version, import result, execution IDs, history refs và credential scope cho một deployment độc lập. Selected-source/topology evidence chưa có, nên BR-14 giữ PARTIAL.
...
BR-18a đã triển khai và review, nội dung được bàn giao qua [PR #90](https://github.com/dvha85/affiliate-expert-learning-roadmap-v2/pull/90): [runbook deployment/recovery 24/7](../architecture/BR-18A-DEPLOYMENT-RECOVERY-RUNBOOK.md) yêu cầu runtime/resource limits, backup checksum, restore/replay, STOP drill và human review. Quyết định hiện tại ngày 21/09/2026 chọn macOS native để kiểm chứng trước, direct runtime Go 1.27/Node 24/n8n 2.38.1, dữ liệu synthetic/read-only, loopback và runtime/backup tách trên `$HOME/affiliate-lab`; về sau vẫn có thể chuyển cùng profile sang Windows + Ubuntu 24.04 LTS trong WSL2 (Hyper-V fallback). Run record `macos-br18a-20260921T155228Z` đã PASS bounded local health/restart/replay/backup/restore/durable-STOP và n8n M06/M07/Schedule Trigger synthetic checks; human review, Windows target/backup host, provider/public deployment và các gap durability vẫn mở. Xem [macOS evidence record](../architecture/EVIDENCE-BR18A-MACOS-LOCAL-LAB-20260921.md) và [Windows deployment decision](../architecture/EVIDENCE-BR18A-WINDOWS-ALWAYS-ON-LAB-20260921.md). `BR-18a` target-host vẫn UNVERIFIED và overall vẫn `NOT_READY_FOR_PRODUCTION`.
...
| 21/09/2026 | Chọn macOS native để kiểm chứng BR-18a trước: Go 1.27/Node 24/n8n 2.38.1, direct runtime, synthetic/read-only, loopback và runtime/backup tách trên `$HOME/affiliate-lab`; giữ Windows + Ubuntu 24.04 LTS (WSL2/Hyper-V) làm phương án always-on về sau | [macOS decision record](../architecture/EVIDENCE-BR18A-MACOS-LOCAL-LAB-20260921.md); [Windows decision](../architecture/EVIDENCE-BR18A-WINDOWS-ALWAYS-ON-LAB-20260921.md); run record/target-host/provider/power-loss/pilot evidence vẫn UNVERIFIED |
| 21/09/2026 | Chạy operated macOS bounded lab với run ID `macos-br18a-20260921T155228Z`: Go/Node/n8n exact versions, loopback health/API, restart/replay, backup/restore, durable STOP và real n8n M06/M07/Schedule Trigger synthetic regressions đều PASS; maintainer review, target-host/provider/public network, Windows parity, power-loss/distributed durability, pilot/live/business evidence vẫn mở | [macOS operated evidence](../architecture/EVIDENCE-BR18A-MACOS-LOCAL-LAB-20260921.md); overall vẫn `NOT_READY_FOR_PRODUCTION` |
