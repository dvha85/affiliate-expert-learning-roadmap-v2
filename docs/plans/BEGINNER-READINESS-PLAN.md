...
BR-11 canary CLI: IN_REVIEW trên baseline `da24ce1`, nhánh `codex/br-11-canary-cli`. `advisor campaign-canary` chỉ một lượt đầu tiên ở fixed campaign path, key từ môi trường, fixture BR-10 duy nhất, khóa toàn lượt và reserve trước request; lượt sau yêu cầu review kể cả lỗi. Có walkthrough đọc report/đối soát phí và phục hồi không reset. Chưa API thật hoặc proof live; BR-11 giữ IN_PROGRESS.
