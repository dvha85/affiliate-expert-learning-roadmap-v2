...
BR-11 BR10 → ledger versioned: IN_REVIEW trên baseline `611d669`. Runner nội bộ tạo fixture qua store BR-10, reserve trước provider, lưu result/v2 với context synthetic đóng băng và accepted output; result/v1 giữ nguyên bytes/digest. Không CLI live, không reset ledger, không API thật. Review tập trung round-trip JSON, mixed-version restart, context tampering, accepted/rejected output và no-refund. BR-11 tổng thể giữ IN_PROGRESS.
