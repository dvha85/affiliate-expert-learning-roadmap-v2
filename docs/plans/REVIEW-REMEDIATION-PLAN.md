...
offline `m11-evaluate` và reviewed `m11-reconcile` bàn giao
record/ledger/evaluation hoặc transition reconciliation đã visible để resolver
hoặc canonical ledger-head kiểm tra trước exact retry. Regression dùng seam sau
file sync, trước parent sync, và kiểm các đường M10, M11 registry, activation,
ledger-init, evaluation và reconciliation. Đây là
