...
func ComputeCanaryCostBoundHash(c CanaryCostBound) string {
	return corem10.ComputeTrustedCostBoundHash(corem10.TrustedCostBound(c))
}

func SealCanaryCostBound(c CanaryCostBound) CanaryCostBound {
	if c.HashVersion == "" {
		c.HashVersion = "go-json-v1"
	}
...
func EvaluateCanaryGate(state M10State, ctx M10Context) CanaryGateDecision {
	...
	if ctx.ApprovedGrantHashes == nil || ctx.TrustedCostBounds == nil {
	...
	if approvedHash != grant.GrantHash {
		g.Reason = "GRANT_APPROVAL_MISMATCH"
		return g
	}
	...
	if strings.TrimSpace(costBound.CostBoundID) == "" {
	...
	if !ok || trustedCostHash != costBound.CostBoundHash {
	...
	g.CostBoundID = costBound.CostBoundID
	g.CostBoundHash = costBound.CostBoundHash
	g.CostBoundMinor = costBound.MaxCostMinor
	...
	if ledger.CostMinorTotal > grant.MaxCostMinorTotal || costBound.MaxCostMinor > grant.MaxCostMinorTotal-ledger.CostMinorTotal {
...
func persistCanaryLedger(path string, ledger CanaryLedger) error {
	b, err := encodeCanaryLedger(ledger)
	if err != nil {
		return err
	}
	tmp := path + ".tmp"
...
func loadExistingCanaryLedger(path string, grant CanaryGrant) (CanaryLedger, error) {
	...
	ledger, err := decodeCanaryLedger(b)
	if err != nil {
		return CanaryLedger{}, err
	}
	if ledger.GrantID != grant.GrantID || ledger.GrantVersion != grant.GrantVersion || ledger.GrantHash != grant.GrantHash {
		return CanaryLedger{}, errors.New("M10 ledger grant binding mismatch")
	}
	return ledger, nil
}
func ensureInitialCanaryLedger(path string, ledger CanaryLedger) error {
	...
	if ledger.ExecutionsTotal != 0 || ledger.ExecutionsInWindow != 0 || ledger.CostMinorTotal != 0 || ledger.PendingOutcomes != 0 || len(ledger.PendingExecutionIDs) != 0 || len(ledger.SuccessfulIdempotencyKeys) != 0 || len(ledger.OutcomeLinks) != 0 || ledger.ReconciliationRequired || ledger.LastExecutionAt != "" {
		return errors.New("M10 initial ledger must be empty")
	}
	if err = reserveCanaryInitialization(path); err != nil {
		return err
	}
...
func ExecuteCanaryLocalSandbox(state *M10State, auth CanaryExecutionAuthorization, ctx M10Context, dir string) (CanaryExecutionRecord, string) {
	...
	if os.IsNotExist(err) {
		if state.Ledger.GrantID != state.Grant.GrantID || state.Ledger.GrantVersion != state.Grant.GrantVersion || state.Ledger.GrantHash != state.Grant.GrantHash {
	...
	normalized.CostMinorTotal += auth.CanaryCostBoundMinor
...
func RecordCanaryOutcome(state *M10State, dir, outcomeID, executionID, observedAt string) string {
	...
	if ledger.GrantHash != state.Grant.GrantHash {
		return "WAIT_RECONCILIATION"
	}
	...
	ledger.OutcomeLinks = append(ledger.OutcomeLinks, CanaryOutcomeLink{OutcomeID: outcomeID, ExecutionID: executionID, ObservedAt: observedAt})
