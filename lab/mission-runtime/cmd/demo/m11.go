...
func normalizedProductionLedger(l ProductionLedger, lease ProductionLease, now time.Time) (ProductionLedger, bool) {
	...
	if l.ControlMode != "NORMAL" && l.ControlMode != "STOPPED" {
		return l, false
	}
	...
	if !safe || lease.WindowSeconds < 60 {
		return l, false
	}
...
func productionGate(state M11State, ctx M11Context) ProductionGateDecision {
	ledgerRaw, _ := json.Marshal(state.Ledger)
	ledgerSum := sha256.Sum256(ledgerRaw)
	return ProductionGateDecision{
		GateID:            "prod-gate-" + state.Lease.LeaseID + "-" + state.Intent.IntentID,
		LedgerArtifactID:  state.Ledger.LeaseID + "/" + state.Ledger.UpdatedAt,
		LedgerContentHash: "sha256:" + hex.EncodeToString(ledgerSum[:]),
...
func validateProductionLeaseApproval(l ProductionLease, ctx M11Context) ([]string, string) {
	...
	if !ok {
		return nil, "LEASE_PROVENANCE_MISSING"
	}
	...
	if !ok || knownHash != l.SourceCanaryGrantHash {
		return nil, "PROMOTION_SOURCE_MISMATCH"
	}
	for _, ref := range approval.SourceE5Refs {
		if !containsFold(ctx.KnownE5Refs, ref) {
			return nil, "PROMOTION_E5_EVIDENCE_MISSING"
		}
	}
	...
	for _, risk := range l.AllowedRiskClasses {
		if !containsFold(approval.ValidatedRiskClasses, risk) {
			return nil, "PROMOTION_RISK_NOT_VALIDATED"
		}
	}
...
func EvaluateProductionGate(state M11State, ctx M11Context) ProductionGateDecision {
	...
	if ctx.TrustedLeaseApprovals == nil || ctx.KnownCanaryGrantHashes == nil || ctx.TrustedHealthSnapshots == nil || ctx.TrustedCostBounds == nil {
		g.Reason = "GOVERNANCE_INPUTS_UNAVAILABLE"
		return g
	}
	validatedRisks, approvalReason := validateProductionLeaseApproval(l, ctx)
	if approvalReason != "" {
		g.Reason = approvalReason
		return g
	}
	if containsFold(ctx.RevokedLeaseIDs, l.LeaseID) {
		return productionStop(&g, "LEASE_REVOKED")
	}
	if ctx.KillSwitch {
		return productionStop(&g, "KILL_SWITCH_ACTIVE")
	}
	...
	if errNow != nil || errReviewed != nil || errValid != nil || errLease != nil || reviewedAt.After(validFrom) || !leaseExpires.After(validFrom) {
		g.Reason = "INVALID_LEASE_TIME"
		return g
	}
	if now.Before(validFrom) {
		g.Decision = "WAIT"
		g.Reason = "LEASE_NOT_YET_ACTIVE"
		return g
	}
	if !leaseExpires.After(now) {
		return productionRequireApproval(&g, "LEASE_EXPIRED")
	}
	...
	if ledger.ControlMode == "STOPPED" {
		return productionStop(&g, "STICKY_STOP")
	}
	if ledger.ReconciliationRequired {
		return productionStop(&g, "RECONCILIATION_REQUIRED")
	}
	if ledger.ConsecutiveFailures >= l.MaxConsecutiveFailures {
		return productionStop(&g, "FAILURE_THRESHOLD")
	}
	...
	if l.PolicyVersion != fresh.PolicyVersion {
		g.Reason = "LEASE_POLICY_MISMATCH"
		return g
	}
	if fresh.RiskClass == "RISK2" {
		return productionRequireApproval(&g, "RISK2_PER_ACTION_APPROVAL_REQUIRED")
	}
	if fresh.RiskClass != "RISK0" && fresh.RiskClass != "RISK1" {
		g.Reason = "UNKNOWN_RISK_CLASS"
		return g
	}
	if fresh.RiskClass == "RISK0" && fresh.Decision != "ALLOW" {
		g.Reason = "POLICY_STATE_NOT_PRODUCTION_ELIGIBLE"
		return g
	}
	...
	if !containsFold(validatedRisks, fresh.RiskClass) {
		g.Reason = "PROMOTION_RISK_NOT_VALIDATED"
		return g
	}
	if !containsFold(l.AllowedRiskClasses, fresh.RiskClass) {
		return productionRequireApproval(&g, "RISK_NOT_DELEGATED")
	}
	...
	if h.HashVersion != "go-json-v1" || h.SnapshotHash == "" || h.SnapshotHash != ComputeProductionHealthHash(h) {
		g.Reason = "TAMPERED_HEALTH"
		return g
	}
	if h.LeaseID != l.LeaseID || h.LeaseVersion != l.LeaseVersion || h.LeaseHash != l.LeaseHash || len(h.SourceRefs) == 0 {
		g.Reason = "HEALTH_MISMATCH"
		return g
	}
	trustedHealthHash, ok := ctx.TrustedHealthSnapshots[h.SnapshotID]
	if !ok || trustedHealthHash != h.SnapshotHash {
		g.Reason = "HEALTH_UNTRUSTED"
		return g
	}
	healthObservedAt, errHealth := time.Parse(time.RFC3339, h.ObservedAt)
	if errHealth != nil || healthObservedAt.After(now) {
		g.Reason = "HEALTH_TIME_INVALID"
		return g
	}
	// Severe known signals must outrank lower-severity DEGRADE signals.
	if h.ComplianceAlertCount > 0 {
		return productionStop(&g, "COMPLIANCE_ALERT")
	}
	if h.ReconciliationRequired {
		return productionStop(&g, "RECONCILIATION_REQUIRED")
	}
	if h.ConsecutiveFailures >= l.MaxConsecutiveFailures {
		return productionStop(&g, "FAILURE_THRESHOLD")
	}
	if h.OldestPendingOutcomeAgeSeconds > l.MaxOutcomeAgeSeconds {
		return productionStop(&g, "OUTCOME_STALE")
	}
	healthAge, safe := checkedMissionSeconds(l.MaxHealthSnapshotAgeSeconds)
	if !safe {
		g.Reason = "INVALID_HEALTH_AGE_LIMIT"
		return g
	}
	if now.After(healthObservedAt.Add(healthAge)) {
		return productionDegrade(&g, "HEALTH_STALE")
	}
	if !h.TelemetryComplete {
		return productionDegrade(&g, "TELEMETRY_INCOMPLETE")
	}
	if h.DependencyState != "HEALTHY" {
		return productionDegrade(&g, "DEPENDENCY_DEGRADED")
	}

	if strings.TrimSpace(c.CostBoundID) == "" {
		g.Reason = "COST_BOUND_MISSING"
		return g
	}
	if c.HashVersion != "go-json-v1" || c.CostBoundHash == "" || c.CostBoundHash != ComputeCanaryCostBoundHash(c) {
		g.Reason = "TAMPERED_COST_BOUND"
		return g
	}
	if c.IntentID != i.IntentID || c.IntentHash != i.IntentHash || c.CorrelationID != i.CorrelationID ||
		...
		g.Reason = "COST_BOUND_MISMATCH"
		return g
	}
	trustedCostHash, ok := ctx.TrustedCostBounds[c.CostBoundID]
	if !ok || trustedCostHash != c.CostBoundHash {
		g.Reason = "COST_BOUND_UNTRUSTED"
		return g
	}
	...
	if ledger.ExecutionsTotal >= l.MaxExecutionsTotal {
		return productionRequireApproval(&g, "PRODUCTION_TOTAL_BUDGET_EXHAUSTED")
	}
	if ledger.ExecutionsInWindow >= l.MaxExecutionsPerWindow {
		g.Decision = "WAIT"
	...
	if ledger.PendingOutcomes >= l.MaxPendingOutcomes {
		g.Decision = "WAIT"
...
func AuthorizeProduction(state M11State, ctx M11Context) (ProductionExecutionAuthorization, ProductionGateDecision, string) {
	...
	if g.Decision != "ALLOW_PRODUCTION" {
		return ProductionExecutionAuthorization{}, g, g.Decision
	}
	now, errNow := time.Parse(time.RFC3339, ctx.Now)
	...
	if errNow != nil || errIntent != nil || errLease != nil || errCost != nil || errHealth != nil {
		g.Decision = "DENY"
		g.Reason = "AUTHORIZATION_TIME_INVALID"
		return ProductionExecutionAuthorization{}, g, g.Decision
	}
...
func InitializeProductionLedger(state *M11State, dir, activatedAt string) string {
	...
	if _, err := encodeProductionArtifact("ledger", state.Ledger); err != nil {
		return "DENY_ACTIVATION"
	}
	if _, err := encodeProductionArtifact("activation", productionActivationRecord{LeaseID: state.Lease.LeaseID, LeaseVersion: state.Lease.LeaseVersion, LeaseHash: state.Lease.LeaseHash, ActivatedAt: activatedAt}); err != nil {
		return "DENY_ACTIVATION"
	}
	...
	if err = writeActivationRecord(activationPath, activation); err != nil {
		return "ACTIVATION_FAILED"
	}
...
func ExecuteProductionLocalSandbox(state *M11State, a ProductionExecutionAuthorization, ctx M11Context, dir string) (ProductionExecutionRecord, string) {
	...
	if err != nil {
		if os.IsExist(err) {
			return ProductionExecutionRecord{}, "WAIT_PRODUCTION_LOCK"
		}
		return ProductionExecutionRecord{}, "EXECUTION_FAILED"
	}
	_ = lock.Close()
	...
	if !productionActivationMatches(activationPath, state.Lease) {
		return ProductionExecutionRecord{}, "STOP_ACTIVATION_STATE_MISSING"
	}
	ledgerPath := productionLedgerPath(dir, state.Lease.LeaseID, state.Lease.LeaseVersion)
	ledger, err := loadProductionLedger(ledgerPath, state.Lease)
	if os.IsNotExist(err) {
		return ProductionExecutionRecord{}, "STOP_LEDGER_MISSING"
	}
	if err != nil {
	...
	if _, err = os.Stat(marker); err == nil {
		if containsFold(state.Ledger.SuccessfulIdempotencyKeys, a.IdempotencyKey) {
			return ProductionExecutionRecord{}, "WAIT_ALREADY_EXECUTED"
		}
		state.Ledger.ReconciliationRequired = true
		if err := persistStickyStop(ledgerPath, state, "RECONCILIATION_REQUIRED", ctx.Now); err != nil {
			return ProductionExecutionRecord{}, "STOP_PERSISTENCE_FAILED"
		}
	...
	if err != nil {
		return ProductionExecutionRecord{}, "STOP_RECONCILIATION"
	}
	...
	if err = f.Close(); err != nil {
		state.Ledger.ReconciliationRequired = true
		if pErr := persistStickyStop(ledgerPath, state, "RECONCILIATION_REQUIRED", ctx.Now); pErr != nil {
			return ProductionExecutionRecord{}, "STOP_PERSISTENCE_FAILED"
		}
	...
	if !ok {
		return uncertain("ledger invalid before durable update")
	}
...
func ResolveProductionReconciliation(state *M11State, dir string, r ProductionReconciliationResolution) string {
	...
	if _, err := time.Parse(time.RFC3339, r.ResolvedAt); err != nil {
		return "DENY_RECONCILIATION_RESOLUTION"
	}
	lockPath := productionLockPath(dir, state.Lease.LeaseID, state.Lease.LeaseVersion)
	lock, err := os.OpenFile(lockPath, os.O_WRONLY|os.O_CREATE|os.O_EXCL, 0600)
	...
	ledgerPath := productionLedgerPath(dir, state.Lease.LeaseID, state.Lease.LeaseVersion)
	ledger, err := loadProductionLedger(ledgerPath, state.Lease)
	if err != nil || ledger.ControlMode != "STOPPED" || !ledger.ReconciliationRequired {
		return "DENY_RECONCILIATION_RESOLUTION"
	}
	if containsFold(ledger.ReconciliationResolutionIDs, r.ResolutionID) {
		return "DUPLICATE_RECONCILIATION_RESOLUTION"
	}
	...
	if err = persistProductionLedger(ledgerPath, ledger); err != nil {
...
func RecordProductionOutcome(state *M11State, dir, outcomeID, executionID, observedAt string) string {
	if state == nil || strings.TrimSpace(outcomeID) == "" || strings.TrimSpace(executionID) == "" {
		return "DENY_OUTCOME"
	}
	lockPath := productionLockPath(dir, state.Lease.LeaseID, state.Lease.LeaseVersion)
	lock, err := os.OpenFile(lockPath, os.O_WRONLY|os.O_CREATE|os.O_EXCL, 0600)
	...
	for _, link := range ledger.OutcomeLinks {
		if link.OutcomeID == outcomeID {
			return "DUPLICATE_OUTCOME"
		}
	}
	idx := -1
	for i, id := range ledger.PendingExecutionIDs {
		if id == executionID {
			idx = i
			break
		}
	...
	if state.Execution != nil && state.Execution.ExecutionID == executionID {
		...
		if err != nil || observed.Before(attempted) {
			return "DENY_OUTCOME"
		}
	}
...
func ValidateProductionClosedCycle(cycle ProductionCycleRecord, state M11State, gate ProductionGateDecision, auth ProductionExecutionAuthorization, exec ProductionExecutionRecord, outcome OutcomeRecord, evaluation EvaluationRecord, proposal *ImprovementProposal, review *ReviewRecord) string {
	...
	if ValidateOutcomeRecord(outcome) != missionValid || outcome.ActionID != exec.ExecutionID {
		return "BROKEN_LINK"
	}
	outcomeAt, _ := time.Parse(time.RFC3339, outcome.ObservedAt)
	execAt, errExec := time.Parse(time.RFC3339, exec.AttemptedAt)
	if errExec != nil || outcomeAt.Before(execAt) {
	...
	if !validResult[evaluation.Result] {
		return missionInvalid
	}
	evaluatedAt, errEval := time.Parse(time.RFC3339, evaluation.EvaluatedAt)
	if errEval != nil || evaluatedAt.Before(outcomeAt) {
		return missionInvalid
	}
	if proposal == nil {
	...
	if EvaluateImprovementProposal(*proposal) == "REJECT_AUTO_APPLY" {
...
func demoM11() (map[string]any, error) {
	...
	if status := InitializeProductionLedger(&state, dir, ctx.Now); status != "INITIALIZED" {
