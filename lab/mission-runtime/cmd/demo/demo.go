...
func runMissionDemo(args []string) error {
	if len(args)!=1{return errors.New("usage: mission-demo O00|M03|M04|M05|M06|M07|M08|M09|M10|M11")}; id:=strings.ToUpper(args[0]); var result any
	switch id{
	...
	case "M04": result=EvaluateAdvisorOutput(AdvisorOutput{State:"ADVISE",Reason:"weak ranking",EvidenceIDs:[]string{"e1"},Unknowns:[]string{}},[]AdvisorEvidence{{EvidenceID:"e1",ObservedAt:"2026-09-03T00:00:00Z",SourceRef:"public"}},"2026-09-03T01:00:00Z",24)
	...
	case "M07": result=EvaluateAgentProposal(demoM07Proposal(),[]ToolSpec{{Name:"public_http",ReadOnly:true,AllowedMethods:[]string{"GET"},AllowedHosts:[]string{"example.com"}}},[]string{"e1"})
