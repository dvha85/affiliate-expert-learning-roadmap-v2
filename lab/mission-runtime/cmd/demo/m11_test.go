...
func TestM11StickyStopPersistsAcrossRestartAndOutcomeStillCloses(t *testing.T){s,c:=baseM11();dir:=t.TempDir();initM11(t,&s,dir,c.Now);a,_,status:=AuthorizeProduction(s,c);if status!="AUTHORIZED"{t.Fatal(status)};s.Authorization=&a;r,status:=ExecuteProductionLocalSandbox(&s,a,c,dir);if status!="EXECUTED"{t.Fatal(status)};s.Health.ComplianceAlertCount=1;refreshHealthTrust(&s,&c);gate,status:=EnforceProductionGate(&s,c,dir);if status!="STOP"||gate.Reason!="COMPLIANCE_ALERT"{t.Fatalf("expected durable STOP got %+v %s",gate,status)};healthy:=s.Health;healthy.ComplianceAlertCount=0;healthy.ObservedAt="2026-09-03T08:01:00Z";healthy=SealProductionHealth(healthy);c.TrustedHealthSnapshots[healthy.SnapshotID]=healthy.SnapshotHash;s.Health=healthy;if gate,status=EnforceProductionGate(&s,c,dir);status!="STOP"||gate.Reason!="STICKY_STOP"{t.Fatalf("STOP must remain sticky after signal clears: %+v %s",gate,status)};if got:=RecordProductionOutcome(&s,dir,"out-stop",r.ExecutionID,"2026-09-03T08:10:00Z");got!="OUTCOME_RECORDED"{t.Fatalf("STOP must not prevent outcome observation: %s",got)}}
