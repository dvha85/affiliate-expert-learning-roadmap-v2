...
type M09Context struct { Now string; KillSwitch bool; Executor ExecutorProfile; AllowedExecutorIDs []string; AlreadySucceeded map[string]bool }
...
func ExecuteLocalSandbox(state M09State, auth ExecutionAuthorization, ctx M09Context, dir string) (ExecutionRecord,string) {
	...
	if !containsFold(ctx.AllowedExecutorIDs,ctx.Executor.ExecutorID) || !executorAllows(ctx.Executor,state.Intent) || ctx.Executor.ExecutorID!="local_sandbox" { return ExecutionRecord{}, "DENY_EXECUTOR" }
...
func demoM09() (map[string]any,error) {
	...
	s:=M09State{Intent:i,Policy:p,Approval:&a}; ctx:=M09Context{Now:"2026-09-03T08:00:00Z",Executor:ExecutorProfile{ExecutorID:"local_sandbox",AllowedActionTypes:[]string{"UPDATE_DRAFT"},AllowedHosts:[]string{"example.com"}},AllowedExecutorIDs:[]string{"local_sandbox"},AlreadySucceeded:map[string]bool{}}
