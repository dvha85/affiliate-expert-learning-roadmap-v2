module missionlab

go 1.23

require github.com/dvha85/affiliate-expert-learning-roadmap-v2/contracts v0.0.0

require github.com/dvha85/affiliate-expert-learning-roadmap-v2/core v0.0.0

require (
	github.com/santhosh-tekuri/jsonschema/v6 v6.0.2 // indirect
	golang.org/x/text v0.14.0 // indirect
)

replace github.com/dvha85/affiliate-expert-learning-roadmap-v2/contracts => ../../contracts

replace github.com/dvha85/affiliate-expert-learning-roadmap-v2/core => ../../core
