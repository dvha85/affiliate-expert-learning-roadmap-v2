...
func ValidateArtifactGraph(entries []ArtifactEntry) error {
	...
	ledgerArtifacts := map[string]ProductionLedger{}
	...
	for _, entry := range entries {
		...
		case *ProductionLedger:
			...
			ledgerArtifacts[entry.ArtifactID] = *x
		...
		case *ProductionGateDecision:
			...
			ledgerState, ledgerStateOK := ledgerArtifacts[x.LedgerArtifactID]
		...
		case *ProductionExecutionAuthorization:
			...
			gateEvaluatedAt, gateTimeErr := time.Parse(time.RFC3339, gate.EvaluatedAt)
