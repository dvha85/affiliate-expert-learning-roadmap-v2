...
- M10: `CanaryGrant` + `CanaryLedger` + `CanaryGateDecision` mở bounded auto-action cho `RISK0/RISK1` trong grant. Grant phải có trusted human approval provenance, exact hash/policy/time/scope/executor/budget; `RISK2` không được delegated. `CanaryGateDecision.execution_authorized=false`; chỉ `ExecutionAuthorization(GOVERNED_CANARY)` riêng mới cấp quyền cho một execution cụ thể.

Contract tồn tại **không tự cấp authority** cho Mission trước hoặc sau. `ALLOW` trong M08 không phải execution permission. `ApprovalRecord` ở M09 cũng không tự là execution. `CanaryGrant` ở M10 không phải blanket approval và không cho Agent/orchestrator quyền tự mở rộng scope/budget/expiry. Schema-valid reference nhưng không resolve được provenance không đủ cho Reality/Operated PASS.
